...
def _foydalanuvchilar() -> dict[str, str]:
    """
    ERP_USERS = "admin:parol1,yordamchi:parol2"
    Иккисига ҳам тўлиқ ҳуқуқ. Парол ичида вергул ва икки нуқта бўлмасин.
    """
    ...
    if xom:
        for juft in xom.split(","):
            ...
            if not juft or ":" not in juft:
                continue
            nom, _, parol = juft.partition(":")
            nom, parol = nom.strip(), parol.strip()
            if nom and parol:
                natija[nom] = parol

    # Эски якка ўзгарувчилар ҳам ишлайверсин
    yakka_nom = os.getenv("ERP_USERNAME", "").strip()
    yakka_parol = os.getenv("ERP_PASSWORD", "").strip()
    if yakka_nom and yakka_parol:
        natija.setdefault(yakka_nom, yakka_parol)

    if not natija:
        raise RuntimeError("ERP_USERS созланмаган. Намуна: admin:parol1,yordamchi:parol2")
    return natija
...
def check_auth(c: HTTPBasicCredentials = Depends(basic)) -> str:
    ...
    kutilgan = FOYDALANUVCHILAR.get(c.username, "")
...
class QoldiqSyncIn(BaseModel):
    ...
    # Тўлиқ сурат бўлса — эскисини тозалаймиз. Бўлакма-бўлак юборилса — йўқ.
    tozalash: bool = False
...
async def sync_qoldiq(
    ...
    await session.commit()
    log.info("Қолдиқ: %s позиция%s", len(payload.qoldiqlar),
             " (тўлиқ сурат)" if payload.tozalash else "")
    return javob({"qabul": len(payload.qoldiqlar)})
